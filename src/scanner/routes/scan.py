...
def _get_set_from_total_count(total_count: int) -> Optional[str]:
    ...
    """
    # XY Set total card counts - all verified unique
    xy_count_mappings = {
        146: ["XY"],  # Base XY set
        106: ["XY Flashfire"],  # Pokemon Center Lady 105/106
        111: ["XY Furious Fists"],  # Korrina 111/111
        119: ["XY Phantom Forces"],  # VS Seeker 109/119
        160: ["XY Primal Clash"],  # Primal Kyogre-EX Alpha ~160
        108: ["XY Roaring Skies", "XY Evolutions"],  # Both have 108 cards
        98: ["XY Ancient Origins"],  # Eevee ~98
        162: ["XY BREAKthrough"],  # Magneton ~162
        122: ["XY BREAKpoint"],  # Greninja 41/122 (verified working)
        124: ["XY Fates Collide"],  # Strong Energy ~124
        114: ["XY Steam Siege"],  # Foongus ~114
    }
    ...
    # For 108 cards: Roaring Skies vs Evolutions  
    if total_count == 108:
        # Evolutions is more recent and has nostalgic base set designs
        logger.info(f"   🎯 Defaulting to XY Evolutions for 108 cards (vs Roaring Skies)")
...
def _correct_xy_set_based_on_number(card_number: str, search_params: Dict[str, Any]) -> Optional[str]:
    ...
    if total_count:
        ...
        if set_from_total:
            ...
            xy_set_ranges = {
                "XY": (1, 146), "XY Flashfire": (1, 106), "XY Furious Fists": (1, 111),
                "XY Phantom Forces": (1, 119), "XY Primal Clash": (1, 160), 
                "XY Roaring Skies": (1, 108), "XY Ancient Origins": (1, 98),
                "XY BREAKthrough": (1, 162), "XY BREAKpoint": (1, 122),
                "XY Fates Collide": (1, 124), "XY Steam Siege": (1, 114), 
                "XY Evolutions": (1, 108)
            }
